...
export async function resolvePrimary<T>(
  param: ResolvePrimaryParam<T>,
): Promise<T> {
  const { db } = param.context;
...
export function toConnectionType<T>(
  items: T[],
  totalCount: number,
  args: IConnectionArgument,
): IConnection<T> {
...
export async function resolveInsert(param: ResolveInsertParam) {
  ...
  const givenId =
    param.lastIdSql !== undefined ? await db.query(param.lastIdSql) : undefined;
  await db.commit();
